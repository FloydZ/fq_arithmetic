...
__global__ void add_fp(limb_t *out) {
    ...
    __fp_add128_32(c[0], c[1], c[2], c[3],
...
__global__ void sub_fp(limb_t *out) {
    ...
    __fp_sub128_32(c[0], c[1], c[2], c[3],
				  a[i][0], a[i][1], a[i][2], a[i][3],
				  b[i][0], b[i][1], b[i][2], b[i][3],
            	  __p[0], __p[1], __p[2], __p[3]);
...
__global__ void schoolbookmul(limb_t *out) {
    int i = blockIdx.x * blockDim.x + threadIdx.x;
    limb_t c[2*LIMBS] = {0}; 
    __school32_4x4(c[0], c[1], c[2], c[3], c[4], c[5], c[6], c[7],
				  a[i][0], a[i][1], a[i][2], a[i][3],
				  b[i][0], b[i][1], b[i][2], b[i][3]);
...
__global__ void mul(limb_t *out) {
    ...
    __mul128_32(c[0], c[1], c[2], c[3], c[4], c[5], c[6], c[7],
				  a[i][0], a[i][1], a[i][2], a[i][3], 
				  b[i][0], b[i][1], b[i][2], b[i][3]);
...
__global__ void sqr(limb_t *out) {
    const uint32_t i = blockIdx.x * blockDim.x + threadIdx.x;
    limb_t c[2*LIMBS] = {0};

    __school_sqr32_4x4(c[0], c[1], c[2], c[3], c[4], c[5], c[6], c[7],
				  a[i][0], a[i][1], a[i][2], a[i][3]);
...
__global__ void schoolbookmullow(limb_t *out) {
    ...
    __school32_4x4_low(c[0], c[1], c[2], c[3],
				  	   a[i][0], a[i][1], a[i][2], a[i][3],
				  	   b[i][0], b[i][1], b[i][2], b[i][3]);
...
__global__ void mul_fp(limb_t *out) {
    ...
    __fp_mul4x4_32_v4(c[0], c[1], c[2], c[3],
				  a[i][0], a[i][1], a[i][2], a[i][3],
				  b[i][0], b[i][1], b[i][2], b[i][3],
...
__global__ void sqr_fp(limb_t *out) {
    ...
    __fp_sqr4x4_32(c[0], c[1], c[2], c[3],
				  a[i][0], a[i][1], a[i][2], a[i][3],
				  __mu[0], __mu[1], __mu[2], __mu[3],
            	  __p[0], __p[1], __p[2], __p[3]);
...
void test_add_fp() {
    ...
    for (uint32_t i = 0; i < TESTS; i++) {
        if (compare(&c[i*LIMBS], add_fp_ok[i])) {
            printf("(%i) FAIL\n", i);
            printf("(%d) 0x%.8lX %.8lX %.8lX %.8lX\n", i, c[i*LIMBS + 3], c[i*LIMBS + 2], c[i*LIMBS + 1], c[i*LIMBS + 0]);
            printf("ok  0x%.8lX %.8lX %.8lX %.8lX\n", add_fp_ok[i][3], add_fp_ok[i][2], add_fp_ok[i][1], add_fp_ok[i][0]);
            return;
        }
    }
    printf("OK\n");
...
void test_sub_fp() {
    ...
    for (uint32_t i = 0; i < TESTS; i++) {
        if (compare(&c[i*LIMBS], sub_fp_ok[i])) {
            printf("(%i) FAIL\n", i);
            printf("(%d) 0x%.8lX %.8lX %.8lX %.8lX\n", i, c[i*LIMBS + 3], c[i*LIMBS + 2], c[i*LIMBS + 1], c[i*LIMBS + 0]);
            printf("ok  0x%.8lX %.8lX %.8lX %.8lX\n", sub_fp_ok[i][3], sub_fp_ok[i][2], sub_fp_ok[i][1], sub_fp_ok[i][0]);
            return;
        }
    }
    printf("OK\n");
...
void test_mul() {
    ...
    for (uint32_t i = 0; i < TESTS; i++) {
        if (compare(&c[2*i*LIMBS], mul_ok[i], 2*LIMBS)) {
            printf("(%i) FAIL\n", i);
            printf("(%d) 0x%.8lX %.8lX %.8lX %.8lX %.8lX %.8lX %.8lX %.8lX\n", i, c[2*i*LIMBS + 7], c[2*i*LIMBS + 6], c[2*i*LIMBS + 5], c[2*i*LIMBS + 4], c[2*i*LIMBS + 3], c[2*i*LIMBS + 2], c[2*i*LIMBS + 1], c[2*i*LIMBS + 0]);
...
void test_sqr() {
    ...
    sqr<<<blocks, threads>>>(c);
    cudaDeviceSynchronize();

    for (uint32_t i = 0; i < TESTS; i++) {
        if (compare(&c[2*i*LIMBS], sqr_ok[i], 2*LIMBS)) {
            printf("(%i) FAIL\n", i);
            printf("(%d) 0x%.8lX %.8lX %.8lX %.8lX %.8lX %.8lX %.8lX %.8lX\n", i, c[2*i*LIMBS + 7], c[2*i*LIMBS + 6], c[2*i*LIMBS + 5], c[2*i*LIMBS + 4], c[2*i*LIMBS + 3], c[2*i*LIMBS + 2], c[2*i*LIMBS + 1], c[2*i*LIMBS + 0]);
            printf("ok  0x%.8lX %.8lX %.8lX %.8lX %.8lX %.8lX %.8lX %.8lX\n", sqr_ok[i][7], sqr_ok[i][6], sqr_ok[i][5], sqr_ok[i][4], sqr_ok[i][3], sqr_ok[i][2], sqr_ok[i][1], sqr_ok[i][0]);
            return;
        }
    }
    printf("OK\n");
...
void test_schoolbookmullow() {
    ...
    for (uint32_t i = 0; i < TESTS; i++) {
        if (compare(&c[i*LIMBS], mul_ok[i])) {
            printf("(%i) FAIL\n", i);
            printf("(%d) 0x%.8lX %.8lX %.8lX %.8lX\n", i,  c[i*LIMBS + 3], c[i*LIMBS + 2], c[i*LIMBS + 1], c[i*LIMBS + 0]);
            printf("ok  0x%.8lX %.8lX %.8lX %.8lX\n", mul_ok[i][3], mul_ok[i][2], mul_ok[i][1], mul_ok[i][0]);
            return;
        }
    }
    printf("OK\n");
...
void test_schoolbookmul() {
    ...
    for (uint32_t i = 0; i < TESTS; i++) {
        if (compare(&c[2*i*LIMBS], mul_ok[i], 2*LIMBS)) {
            printf("(%i) FAIL\n", i);
            printf("(%d) 0x%.8lX %.8lX %.8lX 0x%.8lX %.8lX %.8lX %.8lX %.8lX\n", i, c[2*i*LIMBS + 7], c[2*i*LIMBS + 6], c[2*i*LIMBS + 5], c[2*i*LIMBS + 4], c[2*i*LIMBS + 3], c[2*i*LIMBS + 2], c[2*i*LIMBS + 1], c[2*i*LIMBS + 0]);
            printf("ok  0x%.8lX %.8lX %.8lX 0x%.8lX %.8lX %.8lX %.8lX %.8lX\n", mul_ok[i][7], mul_ok[i][6], mul_ok[i][5], mul_ok[i][4], mul_ok[i][3], mul_ok[i][2], mul_ok[i][1], mul_ok[i][0]);
            return;
        }
    }
    printf("OK\n");
...
void test_mul_fp() {
    ...
    for (uint32_t i = 0; i < TESTS; i++) {
        if (compare(&c[i*LIMBS], mul_fp_ok[i])) {
            printf("(%i) FAIL\n", i);
            printf("(%d) 0x%.8lX %.8lX %.8lX %.8lX\n", i, c[i*LIMBS + 3], c[i*LIMBS + 2], c[i*LIMBS + 1], c[i*LIMBS + 0]);
            printf("ok  0x%.8lX %.8lX %.8lX %.8lX\n", mul_fp_ok[i][3], mul_fp_ok[i][2], mul_fp_ok[i][1], mul_fp_ok[i][0]);
            return;
        }
    }
    printf("OK\n");
...
void test_sqr_fp() {
    ...
    for (i = 0; i < TESTS; i++) {
        if (compare(&c[i], sqr_fp_ok[i])) {
            printf("(%i) FAIL\n", i);
            printf("(%d) 0x%.8lX %.8lX %.8lX %.8lX\n", i, c[i*LIMBS + 3], c[i*LIMBS + 2], c[i*LIMBS + 1], c[i*LIMBS + 0]);
            printf("ok  0x%.8lX %.8lX %.8lX %.8lX\n", sqr_fp_ok[i][3], sqr_fp_ok[i][2], sqr_fp_ok[i][1], sqr_fp_ok[i][0]);
            return;
        }
    }
