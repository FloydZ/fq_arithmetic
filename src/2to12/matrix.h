...
static inline void gf2to12_matrix_add_gf2_u256(gf2to12 *matrix1,
    ...
    for (uint32_t col = 0; col < n_cols; ++col) {
        ...
        while (i >= 8u) {
            const uint16x8_t m2 = gf2to12v_expand_gf2_x8_u128(in2);
...
static inline void gf2to12_matrix_add_scalar_gf2_u256(gf2to12 *matrix1,
    ...
    //         const uint64_t t31 =  t41 ^ (t42 << 32);
    //         const uint64_t t32 =  t43 ^ (t44 << 32);
    //         const uint16x8_t t21  = _mm_set_epi64x(t32, t31);
    //         const uint16x8x2_t t1   = _mm256_cvtepu8_epi16(t21);

    //         const uint16x8x2_t m1 = vld1q_u16_x2((__m256i *)out);
    //         const uint16x8x2_t c1 = gf2to12v_mul_gf2_u256(s256, t1);
    //         vst1q_u16_x2((uint16x8x2_t *)(out +  0), m1 ^ c1);
    ...
    //         const uint64_t t42 = _pdep_u64(mm2[1], m);
    //         const uint64_t t31 = t41 ^ (t42 << 32);
    //         const uint16x8_t t21  = _mm_set_epi64x(0, t31);
    //         const uint16x8_t t1   = _mm_cvtepu8_epi16(t21);
    //         const uint16x8_t c1 = gf2to12v_mul_gf2_u128(s128, t1);
    //         _mm_store_si128((uint16x8_t *)tmp, c1);
    ...
    //         const uint16x8_t t2 = _mm_set_epi64x(t22, t21);
    //         const uint16x8x2_t t1   = _mm256_cvtepu8_epi16(t2);
    //         const uint16x8x2_t m1 = vld1q_u16_x2((__m256i *)out);
    ...
    //         vst1q_u16_x2((uint16x8x2_t *)(tmp +  0), c1);
    ...
    //         const uint64_t t33 = (t43 >> 32) ^ (t44 << 16);
    //         const uint16x8_t t21  = _mm_set_epi64x(t32, t31);
    //         const uint16x8_t t22  = _mm_set_epi64x(0, t33);
    //         const uint16x8x2_t t1   = _mm256_cvtepu8_epi16(t21);
    //         const uint16x8_t t2   = _mm_cvtepi8_epi16(t22);

    //         const uint16x8x2_t m1 = vld1q_u16_x2((__m256i *)out);
    //         const uint16x8_t m2 = vld1q_u16((uint16x8_t *)(out + 16));

    //         const uint16x8x2_t c1 = gf2to12v_mul_gf2_u256(s256, t1);
    //         const uint16x8_t c2 = gf2to12v_mul_gf2_u128(s128, t2);

    //         vst1q_u16_x2((uint16x8x2_t *)(out +  0), m1 ^ c1);
    //         vst1q_u16((uint16x8_t *)(out + 16), m2 ^ c2);
    ...
    //         const uint16x8x2_t t1   = _mm256_cvtepu8_epi16(t21);
    //         const uint16x8x2_t c1 = gf2to12v_mul_gf2_u256(s256, t1);
    //         vst1q_u16_x2((uint16x8x2_t *)tmp, c1);
    ...
    uint16x8_t s = vdupq_n_u16(scalar);
    const uint32_t gf2_col_bytes = gf2_matrix_bytes_per_column(n_rows);
    uint16_t tmp[8] __attribute__((aligned(16))) = {0};
    for (uint32_t col = 0; col < n_cols; ++col) {
        uint32_t i = n_rows;
        ...
        while (i >= 8u) {
            ...
            const uint16x8_t t = gf2to12v_mul_gf2_u128(s, m2);
...
static inline void gf2to12_matrix_mul_gf2_full_v1_u256(gf2to12 *result,
    ...
    for (uint32_t i = 0; i < n_cols1; ++i) {
        ...
        // load the full column
        for (uint32_t j = 0; j < n_rows1 / 8; ++j) {
            A[j] = gf2to12v_expand_gf2_x8_u128(m1 + j);
        }

        // for each col in B
        for (uint32_t j = 0; j < n_cols2; ++j) {
            const uint16x8_t b = vdupq_n_u16(*(matrix2 + j*n_cols1 + i));
            for (uint32_t k = 0; k < n_rows1; k += 8) {
                const uint16x8_t t = gf2to12v_mul_gf2_u128(b, A[k/8]);
                const uint16x8_t c = vld1q_u16((result + j*n_rows1 + k));
                const uint16x8_t d = t^c;
                vst1q_u16((result + j*n_rows1 + k), d);
            }
        }
    }
}
...
static inline void gf2to12_matrix_mul_u256_slim(gf2to12 *result,
    ...
    for (uint32_t col = 0; col < n_cols1; ++col) {
        ...
        gf2to12 *r = result;
        while ((i + 16) <= n_rows1) {
            const uint16x8x2_t a = vld1q_u16_x2(m1);
            uint16x8x2_t t = gf2to12v_mul_u256(a, b);
            uint16x8x2_t t1 = vld1q_u16_x2(r);
            t = veorq_u16_x2(t, t1);
            vst1q_u16_x2(r, t);
        ...
        if (limit) {
            for (uint32_t j = 0; j < limit; ++j) { tmp[j] = m1[j]; }
            const uint16x8x2_t a = vld1q_u16_x2(tmp);
            uint16x8x2_t t = gf2to12v_mul_u256(a, b);
...
static inline void gf2to12_matrix_mul_u256(gf2to12 *result,
    ...
    for (uint32_t j = 0; j < n_cols1; ++j) {
        ...
        if (tail) {
            ...
            A[nr256] = vld1q_u16_x2(tmp);
        }


        // iterate over one row in matrix2
        for (uint32_t i = 0; i < n_cols2; ++i) {
            const uint16x8x2_t b = vdupq_n_u16_x2(matrix2[i*n_cols1 + j]);

            for (uint32_t s = 0; s < nr256; ++s) {
                uint16x8x2_t r = gf2to12v_mul_u256(A[s], b);
                const uint16x8x2_t t1= vld1q_u16_x2((result + i*n_rows1 + s*16));
                r = veorq_u16_x2(r, t1);
                vst1q_u16_x2((result + i*n_rows1 + s*16), r);
            }

            // tail mngt
            if (tail) {
                const uint16x8x2_t r = gf2to12v_mul_u256(A[nr256], b);
                vst1q_u16_x2(tmp, r);
                for (uint32_t k = 0; k < tb; ++k) {
                    result[i*n_rows1 + nr256*16 + k] ^= tmp[k];
                }
