...
static void BM_gf4v_mul_u64(benchmark::State& state) {
    uint64_t a = 1;
    uint8_t b = 2;
    for (auto _ : state) {
        a = gf4v_mul_u64(a, b);
...
#ifdef USE_AVX2
static void BM_gf4v_mul_u256(benchmark::State& state) {
...
BENCHMARK(BM_gf4v_mul_u256);
#endif

BENCHMARK(BM_gf4v_mul_u64);
BENCHMARK_MAIN();
