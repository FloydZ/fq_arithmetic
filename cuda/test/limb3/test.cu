...
__global__ void schoolbookmul(limb_t *out) {
    ...
    __school32_3x3_v2(c[0], c[1], c[2], c[3], c[4], c[5],
				  a[i][0], a[i][1], a[i][2],
				  b[i][0], b[i][1], b[i][2]);
...
__global__ void mul(limb_t *out) {
    ...
    __mul96_32(c[0], c[1], c[2], c[3], c[4], c[5],
				  a[i][0], a[i][1], a[i][2],
				  b[i][0], b[i][1], b[i][2]);
...
__global__ void schoolbookmullow(limb_t *out) {
    ...
    __school32_3x3_low(c[0], c[1], c[2],
				  	   a[i][0], a[i][1], a[i][2],
				  	   b[i][0], b[i][1], b[i][2]);
...
void test_add_fp() {
    ...
    for (uint32_t i = 0; i < TESTS; i++) {
        if (compare(&c[i*LIMBS], add_fp_ok[i])) {
            printf("(%i) FAIL\n", i);
            printf("(%d) 0x%.8lX %.8lX %.8lX\n", i, c[i*LIMBS + 2], c[i*LIMBS + 1], c[i*LIMBS + 0]);
            printf("ok  0x%.8lX %.8lX %.8lX\n", add_fp_ok[i][2], add_fp_ok[i][1], add_fp_ok[i][0]);
            return;
        }
    }
    printf("OK\n");
...
void test_sub_fp() {
    ...
    for (uint32_t i = 0; i < TESTS; i++) {
        if (compare(&c[i*LIMBS], sub_fp_ok[i])) {
            printf("(%i) FAIL\n", i);
            printf("(%d) 0x%.8lX %.8lX %.8lX\n", i, c[i*LIMBS + 2], c[i*LIMBS + 1], c[i*LIMBS + 0]);
            printf("ok  0x%.8lX %.8lX %.8lX\n", sub_fp_ok[i][2], sub_fp_ok[i][1], sub_fp_ok[i][0]);
            return;
        }
    }
    printf("OK\n");
...
void test_mul() {
    ...
    for (uint32_t i = 0; i < TESTS; i++) {
        if (compare(&c[2*i*LIMBS], mul_ok[i], 2*LIMBS)) {
            printf("(%i) FAIL\n", i);
            printf("(%d) 0x%.8lX %.8lX %.8lX 0x%.8lX %.8lX %.8lX\n", i, c[2*i*LIMBS + 5], c[2*i*LIMBS + 4], c[2*i*LIMBS + 3], c[2*i*LIMBS + 2], c[2*i*LIMBS + 1], c[2*i*LIMBS + 0]);
            printf("ok  0x%.8lX %.8lX %.8lX 0x%.8lX %.8lX %.8lX\n", mul_ok[i][5], mul_ok[i][4], mul_ok[i][3], mul_ok[i][2], mul_ok[i][1], mul_ok[i][0]);
            return;
        }
    }
    printf("OK\n");
...
void test_sqr_fp() {
    ...
    for (i = 0; i < TESTS; i++) {
        if (compare(&c[i*LIMBS], sqr_fp_ok[i])) {
            printf("(%i) FAIL\n", i);
            printf("(%d) 0x%.8lX%.8lX%.8lX%.8lX%.8lX%.8lX\n", i, c[5],c[4],c[3],c[2],c[1],c[0]);
            printf("ok 0x%.8lX%.8lX%.8lX%.8lX%.8lX%.8lX\n",  sqr_fp_ok[i][5],sqr_fp_ok[i][4],sqr_fp_ok[i][3],sqr_fp_ok[i][2],sqr_fp_ok[i][1],sqr_fp_ok[i][0]);
            return;
        }
    }
